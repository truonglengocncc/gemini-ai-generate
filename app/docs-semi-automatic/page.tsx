  ...
  const createGroup = async (name?: string) => {
    try {
      const response = await fetch("/api/groups", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: name || `DocsSemi-${Date.now()}` }),
      });
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Paste a sheet (TSV/CSV) with columns: FILE, Prompt, Ratio, Variations, Resolution. Each row = one job; GCS link from FILE column. Results saved to gemini folder at same level.
          </p>
        </div>
